"""Генерация короткого поста для V2 через OpenRouter (по каналу и типу контента)."""
import json
import logging
...
# Системные промпты по каналу (основной контент)
PROMPTS_MAIN = {
...
async def generate_post_v2(
    ...
    """
    Генерирует короткий текст поста. content_type: main, shop, radio, cross, site.
    Возвращает только текст поста (ссылку добавляет вызывающий код).
    """
    settings = get_settings_v2()
    api_key = settings.openrouter_api_key.get_secret_value() if settings.openrouter_api_key else ""
    if not api_key:
        logger.error("[ai_v2] OPENROUTER_API_KEY не задан")
        return None
    ...
    for attempt in range(3):
        try:
            ...
            if content and isinstance(content, str):
                text = content.strip()
                if text:
                    logger.info("[ai_v2] generate_post_v2: успех, channel=%s type=%s len=%s", channel, content_type, len(text))
                    return text
        except Exception as e:
            logger.warning("[ai_v2] OpenRouter попытка %s/3: %s", attempt + 1, e)
    logger.error("[ai_v2] generate_post_v2: не удалось после 3 попыток")
    return None


def fallback_text(title: str, url: str) -> str:
    """Fallback: заголовок + ссылка, если ИИ недоступен."""
    return f"{title}\n\n{url}"
