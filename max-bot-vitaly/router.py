...
async def get_next_item_for_channel(
    ...
    last_sub = None
    if subcategory_gap:
        last_sub_state = await get_rotation_state(f"last_subcategory_{channel}")
        last_sub = str(last_sub_state) if last_sub_state else None
        
    if use_round:
        # Для круговой ротации берём 2000 записей (это недорого) и выбираем по индексу.
        # Если делать SQL OFFSET, нужно знать общее количество записей, что сложнее поддерживать
        rows = await get_catalog_for_channel(channel, exclude_urls_seen_after_ts=cutoff_ts, limit=2000, exclude_subcategory=last_sub)
        if not rows and last_sub:
        ...
        # Для рандомной ротации используем SQL RANDOM(), запрашиваем 1 запись
        rows = await get_catalog_for_channel(channel, exclude_urls_seen_after_ts=cutoff_ts, limit=1, exclude_subcategory=last_sub, order_by_random=True)
        if not rows and last_sub:
            rows = await get_catalog_for_channel(channel, exclude_urls_seen_after_ts=cutoff_ts, limit=1, order_by_random=True)
            
        if not rows:
            return None
            
        chosen = rows[0]
        # Оставляем старую логику апдейта поинтера (чтобы не ломать гибридный режим)
        pointer = await get_rotation_state(f"pointer_{channel}")
        idx = int(pointer) if pointer is not None else 0
        await set_rotation_state(f"pointer_{channel}", value_int=(idx + 1))
