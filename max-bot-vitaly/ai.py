...
def sanitize_llm_plain_text(text: str) -> str:
    """Убирает типичный markdown из ответа модели (**, #, списки) для TG/MAX."""
    t = (text or "").strip()
    if not t:
        return t
    # **жирный** и __жирный__
    ...
    # *курсив* (одна пара на строку, без вложенных *)
    t = re.sub(r"(?<!\*)\*([^*\n]+)\*(?!\*)", r"\1", t)
...
PROMPT_SHOP = """Ты — редактор. Опиши книгу/материал из магазина: что это, кому пригодится, чем полезно. 2–4 предложения. В конце — призыв перейти в магазин (ссылку не пиши). Без восклицаний и капслока. Только plain text, без markdown и звёздочек **."""
PROMPT_RADIO = """Ты — редактор. Напиши короткий атмосферный пост про радио сайта: настроение (вечер, дорога, работа, кофе, дождь — выбери одно), когда включить, зачем перейти. 2–3 предложения. Ссылку не пиши. Без восклицаний. Только сплошной текст, без markdown, без звёздочек ** и без заголовков #."""
PROMPT_CROSS = """Ты — редактор. Напиши нативный анонс другого канала для подписчиков: кому зайдёт, почему стоит перейти. Без слов «подпишитесь» и без восклицаний. 2–3 предложения. Без markdown и звёздочек."""
...
Структура: если материал про место и поездки — место, атмосфера, маршрут или впечатление, зачем послушать или посмотреть перед поездкой.
Если материал не про географию (например, только про напитки, настойки, бар без привязки к поездке) — кратко опиши суть и пользу, не выдумывай маршруты и «локации».
...
Правила: без восклицаний и капслока. Только текст, без markdown. Не выдумывай «подкаст/видео», если в материале только текст про рецепты или напитки — опиши пользу нейтрально.""",
    ...
    for attempt in range(3):
        try:
            ...
            if content and isinstance(content, str):
                text = sanitize_llm_plain_text(content)
                if text:
