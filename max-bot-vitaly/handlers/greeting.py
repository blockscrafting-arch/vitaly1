...
async def on_user_added(event: UserAdded) -> None:
    ...
    if not added_user_id:
        logger.warning("user_added: не удалось получить user_id")
        return
    ...
    try:
        ...
        logger.info("Приветствие отправлено user_id=%s", added_user_id)
    except Exception as e:
        logger.warning("Не удалось отправить приветствие user_id=%s: %s", added_user_id, e)
